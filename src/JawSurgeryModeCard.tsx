...
const JawSurgeryModeCard = ({ onAskDentist, ctaCard, t }: { onAskDentist: (imageSrc: string) => void; ctaCard: React.ReactNode; t: { [key: string]: string } }) => {
    const [file, setFile] = useState<File | null>(null);
    const [previewUrl, setPreviewUrl] = useState<string | null>(null);
    const [surgeryType, setSurgeryType] = useState('Lower Jaw Advancement');
    ...
    const handleGenerate = async () => {
        ...
        const prompt = `You are an expert AI in orthognathic surgical simulation. Based on the user's photo and the specified surgical movement, generate a hyper-realistic, photorealistic prediction of the facial profile. 
        **CRITICAL:** Preserve the user's core identity, skin texture, hair, and all features NOT directly affected by the jaw movement. The change should be anatomically plausible and integrated seamlessly. The lighting must remain consistent.
        **Surgical Movement:** ${surgeryType}.
    ...
    const loadingMessages = [
        ...
        `Simulating ${surgeryType}...`,
        "Adjusting profile contours...",
        "Rendering photorealistic results..."
    ];

    return (
        <div className="card">
            <h2>{t.jawSurgeryPlanner}</h2>
            <p>{t.jawSurgeryDescription}</p>
            ...
            <label htmlFor="surgeryTypeSelect">{t.simulationTypeLabel}</label>
            <select id="surgeryTypeSelect" value={surgeryType} onChange={(e) => setSurgeryType(e.target.value)} style={{marginBottom: '1rem'}}>
                <option>Lower Jaw Advancement</option>
                <option>Lower Jaw Setback</option>
                <option>Upper Jaw Advancement</option>
                <option>Upper Jaw Impaction (for gummy smile)</option>
                <option>Bimaxillary (Both Jaws) Advancement</option>
            </select>
                    ...
                    <div style={{marginTop: '20px'}}>
                        <ComparisonSlider beforeSrc={beforeImage} afterSrc={resultImage} opacity={opacity} />
                         <div className="transparency-slider-container">
                            <label htmlFor="transparency-slider-jaw">{t.transparencyLabel}</label>
                            <input
                                id="transparency-slider-jaw"
                                type="range"
                                min="0"
                                max="1"
                                step="0.01"
                        ...
                        <div className="action-buttons">
                            ...
                            <button onClick={() => { const a = document.createElement("a"); a.href = resultImage; a.download = "Jaw_Surgery_Simulation.png"; a.click(); }} className="icon-btn"><DownloadIcon/></button>
                            <button onClick={() => shareImage(resultImage, "Jaw_Surgery_Simulation.png")} className="icon-btn"><ShareIcon/></button>
...
export default JawSurgeryModeCard;